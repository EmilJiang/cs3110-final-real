...
(** let () = Py.initialize (); let openai = Py.Import.import_module "openai" in
    ...
    "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"); let py_empty_list =
    Py.List.of_list [] in while true do print_endline "type in a question"; let
    s = read_line () in let message = Py.Dict.of_bindings_string
    [
          ("role", Py.String.of_string "user");
          ("content", Py.String.of_string s);
        ]
    in ignore (Py.Object.call_method py_empty_list "append" [| message |]); let
    kwargs =
    [
        ("model", Py.String.of_string "gpt-3.5-turbo");
        ("messages", py_empty_list);
      ]
    in let chat = Py.Module.get openai "chat" in let completions = Py.Module.get
    chat "completions" in let create = Py.Module.get completions "create" in let
