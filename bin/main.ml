...
let create_submit_button input_widget =
  W.button
    ~action:(fun _ -> print_endline (output (W.get_text input_widget)))
    "Submit"

let create_main_layout () =
  let input = create_text_input () in
  let input_and_button = L.flat_of_w [ input; create_submit_button input ] in
  let spacer = L.empty ~w:800 ~h:750 () in
  let layout = L.tower [ spacer; input_and_button ] in
  layout

let setup_gui () =
  let main_layout = create_main_layout () in
  let board = Bogue.of_layout main_layout in
  Bogue.run board
